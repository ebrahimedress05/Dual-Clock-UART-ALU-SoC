...
> **Current stage — First commit:** the system specification and the TSMC13 technology library. The `rtl/` tree is scaffolded (organized by clock domain) but still empty — RTL source will be added in the next commit. The rest of the flow (verification, synthesis, DFT, STA, physical design, signoff, GDS) will follow in later commits.
...
The RTL tree (currently empty, scaffolding only) is organized by **clock domain**, so it's immediately clear which clock will drive each block once the source is added:
...
🚧 **First commit** — system specification (`docs/specs/`) and TSMC13 technology library (`lib/`) added. RTL folder structure is scaffolded and ready; RTL source, testbenches, and the rest of the flow will follow in subsequent commits.
